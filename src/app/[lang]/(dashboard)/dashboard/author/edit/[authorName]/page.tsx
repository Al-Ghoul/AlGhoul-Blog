...
const EditAuthorPage = ({ params }: PageProps) => {
  const { data: authorData, error, isLoading } = useSWR<
    { author: AuthorType[0] }
  >(`/api/authors/${params.authorName}`, fetcher);
  const { data: languagesData } = useSWR<{ languages: LanguageType }>(
    "/api/languages",
    fetcher,
  );
      ...
      <div className="flex-col min-w-full mx-auto lg:mb-16 mb-8 backdrop-blur-3xl bg-blue-800/70 rounded-xl p-3">
        ...
        >
          ...
          <input
            ...
            type="text"
            required
            defaultValue={authorData?.author?.name}
          />
          ...
          <input
            ...
            defaultValue={authorData?.author.profileImageURL}
            className="h-6 rounded-lg bg-blue-200"
            name="profImgURL"
            type="text"
            required
          ...
          <textarea
            ...
            name="bio"
            required
            defaultValue={authorData?.author?.bio}
          />
          ...
          <select
            ...
            name="langId"
            defaultValue={authorData?.author?.languageId}
          >
