...
import type { TagDataType } from "@/helpers/db";
import { editTag } from "@/helpers/actions";


const EditTagPage = ({ params }: PageProps) => {
    const { data, error, isLoading } = useSWR<TagDataType>(`/api/dashboard/tag/${params.id}`, fetcher);
        ...
        <main className="flex min-h-screen p-1 md:p-24 md:px-48">
            <div className="flex-col min-w-full mx-auto lg:mb-16 mb-8 backdrop-blur-3xl bg-blue-800/70 rounded-xl p-3">
                ...
                <form className="flex flex-col mx-auto max-w-screen-sm text-black"
                    ...
                    <input ref={tagInputRef} className="h-6 rounded-lg bg-blue-200" name="tagName" type="text" required defaultValue={data.tag.name} />
                    <label htmlFor="icon" className="text-white">Enter Icon SVG path: </label>
                    <input ref={iconInputRef} className="h-6 rounded-lg bg-blue-200" name="icon" type="text" required defaultValue={data.tag.icon} />
                    <label htmlFor="langId" className="text-white">Select a language: </label>
                    <select ref={languageInputRef} className="text-black rounded-lg bg-blue-200" name="langId" defaultValue={data.tag.languageId} >
