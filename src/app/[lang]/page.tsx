...
async function fetchTopTwoPosts(languadeCode: string) {
  const res = await fetch(
    `${getBaseUrl()}/api/posts/?langCode=${languadeCode}&count=6&include=author,tags&orderBy=date&desc`,
    { next: { tags: ["postsData"] } },
  );
...
export default async function Home({ params }: PageProps) {
  ...
  const { posts } = await fetchTopTwoPosts(params.lang).catch(() => ({
    posts: [],
  }));
  ...
  return (
    <>
      ...
      <main className="flex min-h-screen flex-col items-center">
        <Welcome
          styles={"p-7 mt-20"}
          message={LL.WELCOMING[Math.floor(Math.random() * 2) as 0 | 1]()}
          lanugage={params.lang}
        />
            ...
            <div className="grid gap-8 lg:grid-cols-2">
              {posts?.map((post) => {
                ...
                return (
                  <article
                    key={post.id}
                    className={`p-6 bg-blue-900 rounded-lg border border-gray-200 shadow-md ${className}`}
                  >
                    <div className="flex justify-between items-center mb-5 text-white">
                      ...
                      <span className="text-sm text-gray-300">
                        {`${LL.POSTED_AT()} ${
                          formatDate(post.date.toString(), params.lang)
                        }`}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      <div className="flex items-center">
                        <Link
                          ...
                          href={`/${params.lang}/author/${post.author.name}`}
                        >
