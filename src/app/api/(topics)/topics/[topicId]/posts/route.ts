...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  if (isNaN(topicId))
    return NextResponse.json({ error: true, message: "topicId must be a number identifying a topic" }, { status: 400 });

  const topic = await prisma.topic_Language_Translation.findFirst({
    where: { topicId: topicId, language: { code: languageCode } }
  });
  if (!topic) return NextResponse.json({ message: "Couldn't find this topic" }, { status: 404 });
