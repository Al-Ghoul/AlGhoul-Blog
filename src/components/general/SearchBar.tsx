...
async function searchQuery(url: string, { arg }: { arg: string }) {
    ...
    return (
        <div className="flex flex-col relative">
            <input
                ref={searchInputRef}
                type="text"
                className='rounded-full h-7 bg-blue-950 placeholder:text-gray-300 text-gray-300 md:order-1'
                placeholder='Search...'
                onKeyDown={async (event) => {
                    if (event.key === "Enter" && searchInputRef.current?.value) {
                        await trigger(searchInputRef.current?.value).then(res => setSearchResults(res?.result));
                    }
                }}
            />
            {
