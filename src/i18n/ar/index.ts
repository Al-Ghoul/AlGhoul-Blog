import type { BaseTranslation } from "../i18n-types";

const ar = {
  siteTitle: "الغول",
  copyRightNotice: "{year} © الغول",
  WELCOMING: [
    "أهلا بكم ومرحبا في مدونتي! أنا مطور متحمس لمشاركة معرفتي مع الآخرين. آمل أن تجد مشاركاتي مفيدة وملهمة.",
    "مرحباً بك! أنا مطور وهذه هي مدونتي؛ أكتب عن الأشياء التي أتعلمها، والأشياء التي أقوم ببنائها، والأشياء التي أفكر فيها. آمل أن تجد الأمر ممتعًا!",
  ],
  MY_BLOG: "مدونتي",
  BLOG_ENTRY:
    "هناك الكثير من الموارد لكثير من المواد التعليمية، ومن دواعي امتيازنا أن نكون موجودين في هذا العصر، كُن ملهماً و استخدمه بحكمة.",
  HOME_PAGE: "الرئيسية",
  ABOUT_ME: "عني",
  TAGS_PAGE: "البطاقات",
  AUTHORS_PAGE: "الكُتاب",
  TOPICS_PAGE: "المواضيع",
  READ_MORE: "المزيد",
  SECONDS_AGO: "منذ {seconds} {{لاشئ|ثانية|ثانيتين|ثواني|ثانية}}.",
  MINUTES_AGO: "منذ {minutes} {{لاشئ|دقيقة|دقيقتين|دقائق|دقيقة}}.",
  HOURS_AGO: "منذ {hours} {{لاشئ|ساعة|ساعتين|ساعات|ساعة}}.",
  DAYS_AGO: "منذ {days} {{لاشئ|يوم|يومين|أيام|يوم}}.",
  AUTHOR_POSTS_PAGE: "مقالات {name}",
  ERROR_404: "خطأ 404",
  ERROR_404_PAGE_NOT_FOUND: "لم نتمكن من العثور على هذه الصفحة",
  AUTHOR_NOT_OR_POSTS_FOUND:
    "لم نتمكن من العثور علي هذا الكاتب أو ليس لديه أي مقالات",
  AUTHORS_NOT_FOUND: "لم نتمكن من العثور علي أي كاتب",
  TAG_NO_POSTS_FOUND: "لم نتمكن من العثور أي مقال متعلق بهذه البطاقة",
  TAGS_NOT_FOUND: "لم نتمكن من العثور علي أي بطاقة",
  POST_NOT_FOUND: "لم نتمكن من العثور علي هذا المقال",
  TOPIC_OR_POSTS_NOT_FOUND:
    "لم نتمكن من العثور علي هذا الموضوع أو لا يوجد أي مقالات متعلقة",
  TOPICS_NOT_FOUND: "لم نتمكن من العثور علي أي موضوع",
  SIGN_IN: "تسجيل الدخول",
  SIGN_OUT: "تسجيل الخروج",
  POSTED_AT: "في",
  DESCRIPTION: "مدونة مطور تهدف إلى أن تكون إبداعية وملهمة وتعليمية",
  DESCRIPTION_ABOUT: "تهدف هذه الصفحة إلى سرد مهاراتي بطريقة قصيرة وموجزة",
  DESCRIPTION_AUTHORS:
    "تهدف هذه الصفحة إلى سرد كافة المؤلفين المتاحين في الموقع",
  DESCRIPTION_TAGS: "تهدف هذه الصفحة إلى سرد كافة البطاقات المتاحة",
  DESCRIPTION_TOPICS: "تهدف هذه الصفحة إلى سرد كافة الموضوعات التي نشرنا عنها",
  DESCRIPTION_PROJECTS: "تهدف هذه الصفحة لعرض المشاريع التي قمت بإنجازها.",
  siteTitlePostfix: "مدونة",
  PROJECTS_PAGE: "المشاريع",
  PROJECTS_HEADER: "مشاريع مميزة",
  PROJECTS_INTRO:
    "ألق نظرة علي آخر و أعظم المشاريع التي قمت بوضع الكثير من الشغف و الجهد فيها.",
} satisfies BaseTranslation;

export default ar;
